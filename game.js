...
function animate() {
    ...
    if (controls.isLocked) {
        ...
        if (horizontalMovement.length() > 0) {
            ...
            newPosition.x += forwardVector.x * -horizontalMovement.z + rightVector.x * horizontalMovement.x;
            newPosition.z += forwardVector.z * -horizontalMovement.z + rightVector.z * horizontalMovement.x;
            const playerHorizontalBox = new THREE.Box3().setFromCenterAndSize(
            ...
            for (const block of allBlocks) {
                ...
                if (playerHorizontalBox.intersectsBox(blockBox)) {
        ...
        // Find the lowest block the player is colliding with
        ...
        const onGroundCheck = new THREE.Box3().setFromCenterAndSize(
            new THREE.Vector3(camera.position.x, camera.position.y - playerHeight / 2 - 0.1, camera.position.z),
            new THREE.Vector3(playerRadius * 2, 0.2, playerRadius * 2)
        );
        let onGround = false;
        for (const block of allBlocks) {
            const blockBox = new THREE.Box3().setFromObject(block);
            if (onGroundCheck.intersectsBox(blockBox)) {
                onGround = true;
                break;
            }
        }
