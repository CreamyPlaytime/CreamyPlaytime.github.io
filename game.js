...
function createGrassBlock(x, y, z) {
    ...
    const dirtGeometry = new THREE.BoxGeometry(blockSize, blockSize * 0.9, blockSize);
    const dirtMesh = new THREE.Mesh(dirtGeometry, materials.dirt);
    dirtMesh.position.y = -blockSize * 0.05;
    const grassTopGeometry = new THREE.BoxGeometry(blockSize, blockSize * 0.1, blockSize);
    const grassTopMesh = new THREE.Mesh(grassTopGeometry, materials.grass);
    grassTopMesh.position.y = blockSize * 0.45;
    group.add(dirtMesh);
...
function animate() {
    ...
    if (isPlaying) {
        ...
        if (verticalCollision) {
            ...
            if (velocity.y < 0 && lowestCollisionY > -Infinity) { camera.position.y = lowestCollisionY + playerHeight / 2; velocity.y = 0; canJump = true; } 

            else if (velocity.y > 0 && highestCollisionY < Infinity) { camera.position.y = highestCollisionY - playerHeight / 2; velocity.y = 0; }
